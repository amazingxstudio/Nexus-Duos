...
export default function OpponentProfilePage({ params }: { params: Promise<{ playerId: string }> }) {
  const { playerId } = usePromise(params);
  const token = useAuthStore((s) => s.token);
  const [profile, setProfile] = useState<PublicProfile | null>(null);

  useEffect(() => {
    apiFetch<PublicProfile>(`/profile/${playerId}`, { token }).then(setProfile);
  }, [playerId, token]);
  ...
  return (
    <main className="flex min-h-screen flex-col items-center px-6 pt-12">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="relative">
        ...
        <div
          className="relative flex h-24 w-24 items-center justify-center overflow-hidden rounded-full border-2 border-magenta shadow-glow-magenta bg-surface-raised bg-cover bg-center"
